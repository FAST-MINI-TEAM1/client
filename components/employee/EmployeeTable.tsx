...
interface selectedTapProps {selectedTap:string
  toggle?:boolean
}
...
function EmployeeTable({selectedTap,toggle}:selectedTapProps) {
  ...
  return (
    <> 
      <EmployeeDutyTable> {selectedTap == '전체'? <h1>7월 전체 현황</h1> : selectedTap == '연차' ? <h1>7월 연차 현황</h1> : <h1>7월 당직 현황</h1>}
      <ul>
      {datas.filter(
        (data) =>{
          if(selectedTap == '연차'){return data.orderType == '연차'}
          if(selectedTap == '당직'){return data.orderType == '당직'}
        }
        ).map((data)=>{return(<Employeedata key={data.empName}><Space direction="horizontal" size="middle">
        {data.orderType ==='당직' ? <DutyIcon/> : <AnnualIcon/> }
        <DutyInfo>{data.startDate}</DutyInfo>
        <DutyInfo>{data.status}</DutyInfo>
      </Space></Employeedata>)})} </ul>
